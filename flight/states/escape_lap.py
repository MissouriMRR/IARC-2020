...
class EscapeLap:
    ...
    async def run(self, drone: System):
        """Begins the Escape Lap"""
        logging.info("Starting lap: %d")
        logging.debug("Escape Lap: Straight one")
        await self.wait_pos(drone, config.pylon2)  # move to pylon 2

        logging.debug("Escape Lap: Turn one")
        await self.wait_turn(drone)  # turn around pylon 2

        logging.debug("Escape Lap: Straight two")
