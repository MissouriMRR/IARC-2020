...
RUN_LAPS = 'ApproachLap' #EarlyLaps or ApproachLap
...
async def config_params(drone: System):
    ...
    await drone.param.set_float_param("LNDMC_XY_VEL_MAX", 0.5)
    await drone.param.set_float_param("LNDMC_FFALL_THR", 3)
    await drone.param.set_float_param("LNDMC_FFALL_TTRI", 0.15)
    await drone.param.set_float_param("LNDMC_ALT_MAX", MAX_ALT)
    await drone.param.set_float_param("LNDMC_LOW_T_THR", 0.2)
