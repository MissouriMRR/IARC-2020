...
class TestModuleOrientation(unittest.TestCase):
    ...
    def test_get_module_orientation(self):
        ...
        for current_file in files:
            ...
            if (counter-1) % 2 == 0 and stripped_filename in estimates: # counts every other file and checks if current is in estimates
                ...
                roi = region_of_interest(current_depth, current_depth[current_center[1]][current_center[0]], current_center)
                ...
                self.assertTrue(estimated_pitch*0.9 <= calculated_pitch <= estimated_pitch*1.1) # within ±10%
                self.assertTrue(estimated_yaw*0.9 <= calculated_yaw <= estimated_yaw*1.1) # within ±10%
